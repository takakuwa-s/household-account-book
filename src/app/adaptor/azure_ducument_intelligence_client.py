...
from azure.ai.documentintelligence.models import AnalyzeDocumentRequest, AnalyzeResult, DocumentField, StringIndexType
...
def analyze_receipt(data: bytes) -> list[ReceiptResult]:
    poller: AnalyzeDocumentLROPoller[AnalyzeResult] = document_intelligence_client.begin_analyze_document(
      model_id = "prebuilt-receipt", analyze_request = AnalyzeDocumentRequest(bytes_source=data), string_index_type = StringIndexType.UNICODE_CODE_POINT
    )
    ...
    if receipts.documents:
      for document in receipts.documents:
        ...
        receipt.total = field.get("Total", {}).get("valueCurrency", {}).get("amount", "")
        receipt.items = []
        for value in field.get("Items", {}).get("valueArray", []):
          ...
          item.price = value_object.get("TotalPrice", {}).get("valueCurrency", {}).get("amount", "")
